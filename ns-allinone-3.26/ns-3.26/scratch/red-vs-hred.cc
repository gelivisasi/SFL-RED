...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("queueDiscType", "Set Queue disc type to HRED or ARED", queueDiscType);
  ...
  if ((queueDiscType != "RED") && (queueDiscType != "HRED"))
    {
      std::cout << "Invalid queue disc type: Use --queueDiscType=HRED or --queueDiscType=RED" << std::endl;
      exit (1);
    }
  ...
  if (!modeBytes)
    {
      Config::SetDefault ("ns3::HeuristicRedQueueDisc::Mode", StringValue ("QUEUE_MODE_PACKETS"));
      Config::SetDefault ("ns3::HeuristicRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets));
    }
  else
    {
      Config::SetDefault ("ns3::HeuristicRedQueueDisc::Mode", StringValue ("QUEUE_MODE_BYTES"));
      Config::SetDefault ("ns3::HeuristicRedQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets * pktSize));
      minTh *= pktSize;
      maxTh *= pktSize;
    }
  if(maxTh == 15) maxTh = 3*minTh;
  std::cout << "RedMinth : " << minTh << " RedMaxTh : " << maxTh << std::endl;
  Config::SetDefault ("ns3::HeuristicRedQueueDisc::MinTh", DoubleValue (minTh));
  Config::SetDefault ("ns3::HeuristicRedQueueDisc::MaxTh", DoubleValue (maxTh));
  Config::SetDefault ("ns3::HeuristicRedQueueDisc::LinkBandwidth", StringValue (bottleNeckLinkBw));
  Config::SetDefault ("ns3::HeuristicRedQueueDisc::LinkDelay", StringValue (bottleNeckLinkDelay));
  Config::SetDefault ("ns3::HeuristicRedQueueDisc::MeanPktSize", UintegerValue (pktSize));

  if (queueDiscType == "HRED")
    {
      // Turn on HRED
      Config::SetDefault ("ns3::HeuristicRedQueueDisc::HRED", BooleanValue (true));
      Config::SetDefault ("ns3::HeuristicRedQueueDisc::LInterm", DoubleValue (10.0));
    }
  ...
  HeuristicRedQueueDisc::Stats st = StaticCast<HeuristicRedQueueDisc> (queueDiscs.Get (0))->GetStats ();
