#ifndef HRED_QUEUE_DISC_H
#define HRED_QUEUE_DISC_H
 ...
 *
 * \brief An HRED packet queue disc
 */
...
class qTimeHRed
  {
	...
	uint32_t minTh;
	uint32_t maxTh;
	friend std::ostream& operator<<(std::ostream&, const qTimeHRed&);
  };

std::ostream& operator<<(std::ostream& stream, const qTimeHRed& obj)
{   
    stream << obj.time << ',' << obj.qlen << ',' << obj.minTh << ',' << obj.maxTh;
    return stream;
}

class HeuristicRedQueueDisc : public QueueDisc
{
public:
  ...
  {   
    ...
    std::vector<qTimeHRed> avgQLen;
    std::vector<qTimeHRed> curQLen; 
  } Stats;
   ...
   */
  HeuristicRedQueueDisc::Stats GetStats ();
  ...
  */
  ...
  bool m_isHRED;            //!< True to enable Heuristic RED
  ...
  Time m_targetDelay;       //!< Target average queuing delay in HRED
